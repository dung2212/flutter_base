...
class EnumToString {
  static bool _isEnumItem(enumItem) {
    ...
    return split_enum.length > 1 && split_enum[0] == enumItem.runtimeType.toString();
  }
  ...
  static String convertToString(dynamic enumItem, {bool camelCase = false}) {
    ...
    assert(_isEnumItem(enumItem), '$enumItem of type ${enumItem.runtimeType.toString()} is not an enum item');
  ...
  static String parse(enumItem, {bool camelCase = false}) => convertToString(enumItem, camelCase: camelCase);
  ...
  ///
  static T? fromString<T>(List<T> enumValues, String value, {bool camelCase = false}) {
    try {
      return enumValues
          .singleWhere((enumItem) => EnumToString.convertToString(enumItem, camelCase: camelCase).toLowerCase() == value.toLowerCase());
  ...
  static List<String> toList<T>(List<T> enumValues, {bool camelCase = false}) {
    final _enumList =
        enumValues.map((t) => !camelCase ? EnumToString.convertToString(t) : EnumToString.convertToString(t, camelCase: true)).toList();
  ...
  static List<T?> fromList<T>(List<T> enumValues, List valueList) {
    return List<T?>.from(valueList.map<T?>((item) => fromString(enumValues, item)));
  }
}
